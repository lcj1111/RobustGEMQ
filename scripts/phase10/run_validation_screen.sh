...
run_method() {
  ...
  if [[ -s "$method_dir/validation-items.json" && -s "$method_dir/h6/summary.json" ]]; then
  ...
  HF_DATASETS_OFFLINE=1 TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES="$gpu" \
    "$python_bin" -m pytest tests/test_real_vs_fake_ppl.py tests/test_decode_equiv.py -q -s \
      ...
      --junitxml "$method_dir/h6/junit.xml" > "$method_dir/h6/run.log" 2>&1
  exit_code=$?
  set -e
  "$python_bin" scripts/phase6/write_h6_summary.py --method "$method" \
    --checkpoint "$checkpoint" --exit-code "$exit_code" --junit "$method_dir/h6/junit.xml" \
    --log "$method_dir/h6/run.log" --output "$method_dir/h6/summary.json"
