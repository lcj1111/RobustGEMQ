...
本目录以项目能力链路组织，而不是沿用实验执行时的跳号。脚本、缓存和 `artifacts/` 仍保留原始 `phase0/1/2/3/4/6/9` 名称，以保证已完成实验可以按原命令追溯；文档则使用连续阶段，方便阅读与面试讲解。
...
| 阶段四：鲁棒分配审计 | 验证求解器，筛选待确认的 allocation 假设 | Phase 3 | PIVOT | [报告](04-allocation-audit/report.md) |
...
第一次阅读建议先看 [阶段七发布报告](07-release/report.md)，再顺着阶段六、四和三理解最终结论为何可信；需要验证实现时，使用阶段六 Harness 中保留的原始 `scripts/phase6/*` 命令。
